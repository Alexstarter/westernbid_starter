...
class Westernbid_Starter extends PaymentModule
{
    
    const STARTER_WB_ENABLED = 'PAYMENT_STARTER_WB_ENABLED';

    const STARTER_WB_LOGIN = 'PAYMENT_STARTER_WB_LOGIN';
    const STARTER_WB_SECRETKEY = 'PAYMENT_STARTER_WB_SECRETKEY';

    const MODULE_ADMIN_CONTROLLER = 'AdminConfigurePaymentWesternbid';
    const HOOKS = [
        'paymentOptions',
    ];
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Starter WesternBid Payment Module');
        $this->description = $this->l('Payment Module WesternBid for Prestashop made by STARTER.DESIGN');
    }
    ...
    public function hookPaymentOptions(array $params)
    {
        ...
        if (Configuration::get(static::STARTER_WB_ENABLED)) {
    ...
    private function getExternalPaymentOption()
    {
        ...
        $externalOption->setCallToActionText($this->l('Pay by PayPal'));
        $externalOption->setAction($this->context->link->getModuleLink($this->name, 'external', [], true));

        $externalOption->setAdditionalInformation($this->context->smarty->fetch('module:westernbid_starter/views/templates/front/paymentOptionExternal.tpl'));
    ...
    {
        return (bool) Configuration::updateGlobalValue(static::STARTER_WB_ENABLED, '1');
    }
    ...
    {
        return (bool) Configuration::deleteByName(static::STARTER_WB_ENABLED);
    }
